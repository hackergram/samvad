...
import json
# import pandas
from xetrapal import wakarmas, karma, astra
...
baselogger = astra.baselogger


def get_fileurl(filepath, config=None, logger=baselogger, **kwargs):
    docroot = config.get("Samvad", "urlbase")
    return docroot+"/xetrapal-data"+filepath.split("xetrapal-data")[1]


def validate_vyakti_dict(vyaktidict, new=True, logger=baselogger, **kwargs):
    validation = {}
...
def validate_sandesh_dict(sandeshdict, new=True, logger=baselogger, **kwargs):
    validation = {}
    validation['status'] = True
    validation['message'] = "Valid abhivyakti"
    required_keys = []
...
def create_vyakti(logger=baselogger, **vyaktidict):
...
def create_sandesh(logger=baselogger, config=None, path=None, **sandeshdict):
    if validate_sandesh_dict(sandeshdict)['status']:
        ...
        if sandesh != []:
            if sandesh[0].created_timestamp.replace(microsecond=0) == sandeshdict['created_timestamp'].replace(microsecond=0, tzinfo=pytz.UTC):
...
def search_sandesh(logger=baselogger, config=None, path=None, **kwargs):
...
def sighted_abhivyakti(abhivyakti, logger=baselogger, **kwargs):
...
def fb_get_profile_data(fbbrowser, url, logger=baselogger, **kwargs):
    profiledata = {}
    profilepic = {}
    fbbrowser.get(url)
    karma.wait()
...
def fb_get_profile_tab_data(fbbrowser, profileurl, logger=baselogger, **kwargs):
...
def fb_get_cur_page_displayname(fbbrowser, logger=baselogger, **kwargs):
    displayname = fbbrowser.find_element_by_id("fb-timeline-cover-name").find_element_by_tag_name("a").text
    return displayname


def fb_like_page_toggle(fbbrowser, pageurl, logger=baselogger, **kwargs):
    fbbrowser.get(pageurl)
    likebutton = fbbrowser.find_element_by_xpath("//button[@data-testid='page_profile_like_button_test_id']")
    likebutton.click()


def wa_update_samvad_sandesh(wabrowser, samvad, historical=False, logger=baselogger, scrolls=10, **kwargs):
    logger.info("Searching for messages in samvad {}".format(samvad.naam))
    p = wakarmas.wa_get_conv_messages(wabrowser, samvad.naam, historical=historical, logger=logger, scrolls=scrolls)
    messages = []
    seen_messages = []
    # return messages
    for message in p:
        m = wakarmas.wa_get_message(wabrowser, message, logger=logger)
...
def create_sandesh_graph(logger=baselogger, **kwargs):
    logger.info("Trying to create sandesh")
    for sandesh in documents.Sandesh.objects:
        sandeshdict = json.loads(sandesh.dump_dict())
        create_sandesh(sandeshdict)


def export_query(query, logger=baselogger, path=".", filename="query.csv", dataframe=False, **kwargs):
    rows, colnames = samvadgraphmodel.db.cypher_query(query)
    colnames = [colname.split(".")[1] for colname in colnames]
    rowdicts = []
    for row in rows:
        rowdict = {}
        for key, value in zip(colnames, row):
            if "timestamp" in key:
                rowdict[key] = datetime.datetime.fromtimestamp(value).replace(microsecond=0)
            else:
                rowdict[key] = value
        rowdicts.append(rowdict)
    rowdf = pandas.DataFrame(rowdicts)
    if dataframe is True:
        return rowdf
    filepath = karma.export_table(rowdf, filename=filename, path=path, logger=logger)
    logger.info("Exported query {} to {}".format(query, filepath))
    return filepath
